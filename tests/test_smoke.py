...
def test_compute_baseline_projections_ownership_column(synthetic_players: pd.DataFrame) -> None:
    projections = compute_baseline_projections(synthetic_players)
    # In this tree, ownership is estimated downstream by
    # slate_optimizer.projection.ownership_model — baseline only guarantees
    # the column exists and is non-negative.
    assert "proj_fd_ownership" in projections.columns
...
def test_mlb_api_fetch_structure(monkeypatch) -> None:
    """Mock MLB Stats API response and verify fetch_batting_orders returns correct columns."""
    import unittest.mock as mock
    from datetime import date

    from slate_optimizer.ingestion import mlb_lineups_api as mlb_api
        ...
        "dates": [
            {
                "games": [
                    {
                        "teams": {
                            "home": {"team": {"name": "New York Yankees"}, "probablePitcher": {"fullName": "Gerrit Cole"}},
                            "away": {"team": {"name": "Boston Red Sox"}, "probablePitcher": {"fullName": "Chris Sale"}},
                        },
                        "lineups": {
                            "homePlayers": [
    ...
    with mock.patch.object(mlb_api, "_api_get", return_value=fake_response):
        df = mlb_api.fetch_batting_orders(game_date=date(2026, 4, 15))

    assert set(df.columns) >= {"team", "order_position", "player_name"}
    batters = df[df["order_position"] >= 1]
    pitchers = df[df["order_position"] == 0]
    assert len(batters) == 18  # 9 home + 9 away
    assert set(batters["team"].unique()) == {"NYY", "BOS"}
    assert batters["order_position"].max() == 9
    assert len(pitchers) == 2
    assert set(pitchers["player_name"]) == {"Gerrit Cole", "Chris Sale"}


def test_odds_api_no_key(monkeypatch) -> None:
    """fetch_vegas_lines raises ValueError when no API key is set."""
    from slate_optimizer.ingestion.vegas_api import fetch_vegas_lines

    monkeypatch.delenv("ODDS_API_KEY", raising=False)
    with pytest.raises(ValueError):
        fetch_vegas_lines(api_key=None)
